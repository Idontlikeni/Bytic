...
var quizApp = function() {
	...
	this.qno = 1;
	this.currentque = 0;
	var totalque = quiz.JS.length;//передача параметров в totalque
...
var selectedopt;
	...
	$('#question-options').on('change', 'input[type=radio][name=option]', function(e) {
			...
			$(this).prop("checked", true);//добавим checked
				selectedopt = $(this).val();//передаем текущее значение радиокнопки<Да> или <Нет> в selectedopt
...
$('#next').click(function(e) {
			e.preventDefault();
			if(selectedopt) {//проверка есть состояние радиокнопки или нет
				console.log(selectedopt);
				jsq.checkAnswer(selectedopt);
				if(selectedopt == "<Нет>"){
					jsq.changeQuestion(2);
				}else{
	...
	$('#previous').click(function(e) {//проверка есть состояние радиокнопки или нет
		e.preventDefault();
		if(selectedopt) {
