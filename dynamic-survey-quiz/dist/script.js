jQuery('.mm-prev-btn').hide();
	...
	function getCount() {
		...
		count = jQuery('.mm-survey-page').length - num_of_branches;
		return count;
	...
	function goToNext() {
		...
		jQuery('.mm-next-btn').on('click', function() {
			...
			jQuery('.mm-survey-item input:checked').each(function(index, val) {
				...
				console.log(index, data, name, n, total, x);
				if(index == x % 10 - 1){
					if(data == 'Нет' && index == 0){
						branch = 2;
						flag = true;
			...
			current = x % 10 + 1 + branch * 10;
			console.log(current, x, count);
			var g = (current - branch * 10)/count;
			buildProgress(g);
			var y = (count + 1);
			getButtons();
	...
	function collectData() {
		...
		jQuery('.mm-survey-item input:checked').each(function(index, val) {
			...
			map[name] = data;
			console.log(index, data, name, n, total);
		});
		...
		for (i = 1; i <= count; i++) {
			...
			g = ans[i - 1]
			jQuery('.mm-survey-results-list').append('<li class="mm-survey-results-item '+'correct'+'"><span class="mm-item-number">'+i+'</span><span class="mm-item-info">'+g+'</span></li>');
